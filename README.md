...
研究方向有變動時（新題目、某個領域退燒）也是改這裡，不要只加關鍵字。

**太多雜訊** → 先調 `thresholds.read`，再看 `read_cap`。

## 設定鍵速查

| 鍵 | 作用 |
|---|---|
| `profile` | 打分的唯一依據。方向變了就改這段，效果遠大於調參數 |
| `rubric` | 各分數級距的定義，控制模型給分的鬆緊 |
| `why_instruction` | 中文說明的寫法。目前要求只寫結論＋數字，不准提跟你的關聯 |
| `thresholds.read` / `.scan` | 進「值得讀」「掃一眼」的分數門檻 |
| `read_cap` | 「值得讀」的硬上限。溢出的降到「掃一眼」，不會消失。比調門檻可靠——就算模型某天普遍給高分，清單長度仍然固定 |
| `also_exclude` | 排除另一頁「已經顯示出來過」的論文。只算真的出現在值得讀／掃一眼的，低分被埋的不算，所以同一篇在研究雷達 2 分、在教學版仍可能拿 9 分 |
| `tags` | 分類標籤。`group_by: tag` 時會用來分組 |
| `max_score_items` | 單次送去打分的上限，控制 API 成本 |
